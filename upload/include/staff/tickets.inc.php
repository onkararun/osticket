...
$sortOptions=array('date'=>'effective_date','ID'=>'ticket.`number`*1',
    ...
    'dept'=>'dept.dept_name','update'=>'ticket.updated');
...
$qselect ='SELECT ticket.ticket_id,tlock.lock_id,ticket.`number`,ticket.dept_id,ticket.staff_id,ticket.team_id '
    ...
    .' ,email.address as email, dept.dept_name, status.state '
         .' ,status.name as status,ticket.source,ticket.isoverdue,ticket.isanswered,ticket.created,ticket.updated ';/** query for fetch data form spr_ticket table  **/
 ...
 <table class="list" border="0" cellspacing="1" cellpadding="2" width="940">
    <thead>
        <tr>
            ...
            <th>
                <a  <?php echo $update_sort; ?> href="tickets.php?sort=update&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
                    title="<?php echo sprintf(__('Sort by %s %s'), __('Date'), __($negorder)); ?>"><?php echo __('Last Updated'); ?></a>
            </th><!-- last updated column heading which we created -->
            <?php
            if($search && !$status) { ?>
                <th width="60">
            ...
            <tr id="<?php echo $row['ticket_id']; ?>">
                ...
                <td align="center" nowrap>
                  <?php 
                    ...
                    //echo date('d-M-y h:i a', strtotime($row['effective_date']));
                  ?>
                </td>
              ...
              <td nowrap>&nbsp;<?php echo Format::htmlchars(
               ...
               <td align="center" nowrap>
                    <?php 
                      ...
                      $updated = $ticket->getLastMessageUpdatedDate();
                      //echo date('d-M-y h:i a', strtotime($updated));
                      echo Format::db_datetime($updated);
                    ?>
                </td><!-- last updated column which we created-->
                <?php
                if($search && !$status){
